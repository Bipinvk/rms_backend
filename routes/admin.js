...
router.get("/project/:id/resources", async (req, res) => {
  const resources = await Resource.find({ project_id: req.params.id }).populate(
    "user_id",
    "name email"
  );
  res.json(resources);
...
router.put('/project/:id/documents', handleUpload, async (req, res) => {
  if (!req.fileUrl) return res.status(400).json({ msg: 'No file URL generated' });
  ...
  res.json({ project, message: 'Document uploaded successfully' });
});
